...
void balancear(No* bisavo, No* avo, No* pai, No* i, ArvoreBicolor *a) {
    if (pai != NULL && pai->cor == true) {
        if (strcmp(pai->elemento->nome, avo->elemento->nome) > 0){
            if(strcmp(i->elemento->nome , pai->elemento->nome) > 0){
        ...
        if(bisavo == NULL){
            a->raiz = avo;
        } else if(strcmp(avo->elemento->nome, bisavo->elemento->nome) < 0){
            bisavo->esq = avo;
        } else {
            bisavo->dir = avo;
        }
...
void inserirB(Restaurante *x, No* bisavo, No* avo, No* pai, No *i, ArvoreBicolor *a) {
    if(i == NULL){
        if(strcmp(x->nome, pai->elemento->nome) < 0){
           i = pai->esq = novoNo(x);
        } else {
           i = pai->dir = novoNo(x);
        }
        ...
        if(strcmp(x->nome, i->elemento->nome) < 0){
...
void inserirRec(ArvoreBicolor *a, Restaurante *x) {
    if (a->raiz == NULL) {
        ...
        if(strcmp(x->nome, a->raiz->elemento->nome) < 0){
            a->raiz->esq = novoNo(x);
        } else {
            a->raiz->dir = novoNo(x);
        }
        ...
        if(strcmp(x->nome, a->raiz->elemento->nome) < 0){
            a->raiz->esq = novoNo(x);
        }else if (strcmp(x->nome, a->raiz->dir->elemento->nome) < 0){
        ...
        if(strcmp(x->nome, a->raiz->elemento->nome) > 0){
            a->raiz->dir = novoNo(x);
        }else if(strcmp(x->nome, a->raiz->esq->elemento->nome) > 0){

...
class ListaDupla{
    ...
    public CelulaDupla getAt(int pos){
...
public class Principal{
   ...
   public static void swap(ListaDupla ld, int i, int j){
        CelulaDupla ci = ld.getAt(i);
        CelulaDupla cj = ld.getAt(j);
        Restaurante aux = ci.rest;
        ci.rest = cj.rest;
        cj.rest = aux;
        mov += 3;
    }
 
    public static void quickRec(ListaDupla r, int esq, int dir) {
        int i = esq, j = dir;
        int meio = (esq + dir) / 2;

        double pivo = r.getAt(meio).rest.getAvaliacao();
        String pivoNome = r.getAt(meio).rest.getNome();
 
        while (i <= j) {
            comp++;
            while (r.getAt(i).rest.getAvaliacao() < pivo ||
                  (r.getAt(i).rest.getAvaliacao() == pivo &&
                   r.getAt(i).rest.getNome().compareTo(pivoNome) < 0)) {
                comp++;
                i++;
            }
            comp++;
            while (r.getAt(j).rest.getAvaliacao() > pivo ||
                  (r.getAt(j).rest.getAvaliacao() == pivo &&
                   r.getAt(j).rest.getNome().compareTo(pivoNome) > 0)) {
                comp++;
                j--;
            }
        ...
        if (i < dir)  quickRec(r, i, dir);
    }   

    public static void Quick(ListaDupla ld, int n) {
        quickRec(ld, 0, n - 1);
    }
        public static void main(String[] args) throws Exception{
            ...
            FileWriter arq = new FileWriter("880222_quicksort_flexivel.txt");

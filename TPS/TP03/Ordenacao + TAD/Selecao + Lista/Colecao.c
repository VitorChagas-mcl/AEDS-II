...
{ // criei uma funcao para liberar os vetores alocados
    free(r->nome);
    free(r->cidade);
...
{                                                    // funcao para cria a colecao
    FILE *arq = fopen("/tmp/restaurantes.csv", "r"); // abro o arquivo para leitura

    if (arq == NULL)
    { // verifico se o ponteiro do arquivo nao é nulll
        printf("Erro ao abrir arquivo!");
    ...
    { // verifico se a alocacao deu certo
...
{ // funcao para buscar o restaurante
    for (int i = 0; i < colecao->tamanho; i++)
    { // pego o tamanho da colecao
        if (colecao->restaurante[i].id_restaurante == id_buscado)
        {             // e procuro o id buscado do restaurante
...
{ // funcao para transforma o char em int
    int qtdCaracteres;
...
int main()
{
    ...
    FILE *arq_log = fopen("880222_selectionsort_filaflexivel.txt", "w");

...
void Insercao(Restaurante r[], int n, int k, int* comp, int* mov){
    ...
    for(int i = 0; i < n; i++){
        ...
        int j = (i < k) ? i - 1 : k - 1;
        (*comp)++;
        while((j >= 0 && strcmp(aux.cidade, r[j].cidade) < 0)){
            (*mov)++;
            r[j + 1] = r[j];
            j--;
            (*comp)++;
...
int main(){
   /*pequeno teste para ver se esta funcionando
    ...
    inicio = clock();
    Insercao(r_ordenados, ordenados, 10, &comp, &mov);
    fim = clock();
    ...
    FILE* arq_log = fopen("880222_insercao_parcial.txt", "w");

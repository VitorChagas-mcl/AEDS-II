...
int maior_data(Restaurante* r1, Restaurante* r2) {
    ...
    return strcmp(r1->nome, r2->nome) > 0; // desempate por nome
}
...
void heapsort(Restaurante* rest, int n) {
    // desloca tudo 1 posição para usar índice 1-based
...
int main(){
   /*pequeno teste para ver se esta funcionando
    ...
    inicio = clock();
    heapsort(r_ordenados, ordenados);
    fim = clock();
    ...
    FILE* arq_log = fopen("880222_heapsort_parcial.txt", "w");
